...
Infer.__init = argcheck{
  ...
  call =
    function(self, np, scales, meanstd, model, iSz, dm, timer)
      ...
      self.topScores, self.topMasks, self.topMotionMasks = torch.Tensor(), torch.ByteTensor(), torch.FloatTensor()
      local topPatches
      if self.dm then
...
function Infer:getTopPatches(outPyramidTrunk,outPyramidSkip)
  ...
  for i = 1, self.np do
    ...
    for j = 1, #self.skpos do
      ...
      -- upsampling based on how far down the network the skip layer is located
...
--[[ 
Inputs: 
 	thr: 	Threshold over which we will consider a 
		pixel containing an object. The tensor after 
		thresholding will be used to calculate measures such as IoU.
	h, w: 	height, width of the input frame 
  frame_file: 	The file name corresponding to the frame 'f' 
		that is being segmented We will use this name to store our masks. 
 motion_file: 	The full path to file containing motion segmentation from Pia's
	        algorithm for frame 'f'. 

Output: 
   topMasks :	This will contain the top motionMasks 

  Definition:
	Let motionMasks := masks that have been segmented 
			   using motion files from Pia's algorithm
]]

local topMasks = torch.ByteTensor()
local imgMask = torch.ByteTensor()
--We will store each motionMask in the tensor 
-- motionMask and save it to disk when 

local topMotionMasks = torch.FloatTensor()
local motionMask = torch.FloatTensor()
local topScoresFinal = torch.FloatTensor()

function Infer:getTopMasks(thr,h,w, frame_file, motion_file)
  ...
  topMotionMasks:resize(np,h,w):zero()
  topScoresFinal:resize(np):zero()
  imgMask:resize(h,w)
  motionMask:resize(h,w)
  local imgMaskPtr = imgMask:data()
  local motionMaskPtr = motionMask:data()

  -- crt_ptr: current_pointer to the last mask saved in topMasks
  local crt_ptr = 1
  --motion_q will hold the motion based segmentation of Pia's matrix. 
  local motion_q = image.load(motion_file, 1, byte)
  motion_q:apply(function(x)
           if x > 0.99 then x = 0
           else x = 1
           end return x end )

  motion_q = motion_q:byte()

  for i = 1,np do --i = 1,np do
    imgMask:zero()
    motionMask:fill(0.00)
    local score, scale,x,y =topScores[i][1], topScores[i][2],topScores[i][3],topScores[i][4]
    ...
    for im =0, sz-1 do
      ...
      for jm = 0,sz- 1 do
        ...
        if ii >=0 and ii <= h-1 and jj >=0 and jj<=w-1 then
          motionMaskPtr[jj+w*ii]= maskPtr[sz*im+jm] end
      end
    end
    -- Store the # of object pixels in each mask.  
    s1 = motion_q:sum()
    s2 = imgMask:sum()
   
    --Intersection of motion_q and the image mask.  
    local I = motion_q:maskedSelect(imgMask)
   
    --Lets set alpha and beta 
    local a, b = 0, 0
    if s1 > s2 then a = 0.8 else a = 0.8*(s1/s2) end
    if s1 > s2 then b = 0.1 else b = 0.1 *(s1/s2) end

    --Check if we have a good mask
    if (I:sum()/s2) > a and (I:sum()/(s1+s2)) > b then 
        is_rpt = 0 -- is imgMask similar to already chosen mask?    
        rep_idx = 0 -- index of mask to be replaced
        for j=1,crt_ptr-1 do
          s3 = topMasks[j]:sum()
          I_jc =  topMasks[j]:maskedSelect(imgMask) 
          if (I_jc:sum()/(s3 + s2)) > 0.2 then
             is_rpt = 1 
             I_jm = motion_q:maskedSelect(topMasks[j])
             if (I_jm:sum()/(s3 + s1)) < (I:sum()/(s1 + s2)) then 
		rep_idx = j 
                break
	     end 
          end
        ...
        if is_rpt == 0 then 
          topMasks:narrow(1,crt_ptr,1):copy(imgMask) 
          topMotionMasks:narrow(1,crt_ptr,1):copy(motionMask)
          topScoresFinal[crt_ptr] = score
          crt_ptr = crt_ptr + 1
        elseif is_rpt == 1 and rep_idx ~=0 then
          topMasks:narrow(1,rep_idx,1):copy(imgMask)
          topMotionMasks:narrow(1,rep_idx,1):copy(motionMask)
          topScoresFinal[rep_idx] = score
        end
    end
  end

  self.topMasks = topMasks
  self.topMotionMasks = topMotionMasks
  return self.topMasks, topScoresFinal
end
...
--[[ 
Inputs:
     motion:    Boolean value. If motion == 1, then we will return the topMotionMasks. 
                               If motino == 0, then we will return the topMasks. 
 	thr: 	Threshold over which we will consider a 
		pixel containing an object. The tensor after 
		thresholding will be used to calculate measures such as IoU.
	h, w: 	height, width of the input frame 
...
Output: 
   topMasks, topScores: topMotionMasks will contain the top binary masks. 
                        topScores[i] will contain the score for topMasks[i]
                        topMotinoMasks will contain the top motinoMasks
         or 
   topMotionMasks, topScores: 
]]
function Infer:getTopProps(motion,thr,h,w, filename, motion_file)
  _, topScores = self:getTopMasks(thr,h,w, filename, motion_file)
  if motion == 0 then return self.topMasks, topScores
  else return self.topMotionMasks, topScores
  end
end
--------------------------------------------------------------------------------
