...
function Infer:getTopMasks1(thr,h,w, frame_file, objects)
  ...
  imgMask:resize(h,w)
  --testerMask:resize(h,w):zero()
  local imgMaskPtr = imgMask:data()

  for i = 1,objects do --i = 1,np do
    imgMask:zero()
...
local topScoresFinal = torch.FloatTensor()
local testerMask = torch.ByteTensor()
function Infer:getTopMasks2(thr,h,w, frame_file, motion_file)
  thr = math.log(thr/(1-thr)) -- 1/(1+e^-s) > th => s > log(1-th)
...
function Infer:getTopProps1(thr,h,w, filename, objects)
  _, topScores = self:getTopMasks1(thr,h,w, filename, objects)
  return self.topMasks, topScores
  end


function Infer:getTopProps2(thr,h,w, filename, motion_file)
  _, topScores = self:getTopMasks2(thr,h,w, filename, motion_file)
  return self.topMotionMasks, topScores
  end
